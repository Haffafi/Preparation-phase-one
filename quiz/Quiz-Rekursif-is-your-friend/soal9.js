...
function virusCheckRecursive(str, viruses) {
  str = str.toLowerCase();
  if (viruses == null) return "There is no virus";
  if (viruses.length == 0) return 0;
  else {
    viruses = viruses.toLowerCase();
    let virus = viruses[0];
    let result = new RegExp(virus, "g");
    let banyak = str.match(result);
    if (banyak == null) {
      return virusCheckRecursive(str, viruses.substring(2));
    } else {
      return banyak.length + virusCheckRecursive(str, viruses.substring(2));
    }
  }
}
